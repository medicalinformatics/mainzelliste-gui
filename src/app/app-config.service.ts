...
import {
  MainzellisteField,
  MainzellisteFieldType,
} from "./model/mainzelliste-field";
...
export class AppConfigService {
  ...
  private mainzellisteAssociatedIdGeneratorsMap: Map<string, IdGenerator[]> =
    new Map<string, IdGenerator[]>();
      ...
      this.httpClient
        ...
        .subscribe({
          next: (r) => {
            ...
            // init feature toggle
            this.copyConcatenatedIdEnabled =
              this.data[0].betaFeatures?.copyConcatenatedId ?? false;
            this.copyIdEnabled = this.data[0].betaFeatures?.copyId ?? false;
            this.copyConcatenateSeparation =
              this.data[0].betaFeatures?.copyConcatenateSeparation ?? ".";
            this.configurationEnabled =
              this.data[0].betaFeatures?.configuration ?? false;
            this.disableBulkIdGeneration =
              this.data[0].betaFeatures?.disableBulkIdGeneration ?? false;
            this.disableIdSelection =
              this.data[0].betaFeatures?.disableIdSelection ?? false;
            this.onkostarUrl = this.data[0].betaFeatures?.onkostarUrl ?? "";
            this._showDomainsInIDCard =
            ...
            if (
              ...
              this.data[0].genderFieldValues.length == 0
            )
              this.data[0].genderFieldValues =
                PatientList.defaultFenderFieldValues;

            // init layout
            this.layoutFooterLogos = this.data[0].layout?.footerLogos ?? [];

            //start validation
            this.validateBackendUrl(this.data[0]).subscribe({
            ...
            reject(
              new Error(
                this.translate.instant(
                  "error.app_config_service_config_not_found",
                ),
              ),
            ),
        });
  ...
  public fetchVersion(): Promise<{ distname: string; version: string }> {
    return lastValueFrom(
      this.httpClient
        .get<{ distname: string; version: string }>(this.data[0].url + "/", {
            ...
            return throwError(
              () =>
                new MainzellisteUnknownError(
                  this.translate.instant(
                    "error.patient_list_service_get_version",
                  ),
                  e,
                  this.translate,
                ),
  ...
  private validateBackendUrl(config: PatientList) {
    ...
    let urlSuffix = new URL(config.url.toString()).pathname.endsWith("/")
      ? ""
      : "/";
    return this.httpClient.get<string>(config.url.toString() + urlSuffix).pipe(
      map((_r) => this.translate.instant("appConfigService.backend_online")),
      catchError((_e) =>
        throwError(
          () =>
            new Error(
              this.translate.instant(
                "error.app_config_service_backend_offline",
              ),
            ),
        ),
      ),
  ...
  public fetchMainzellisteIdGenerators(): Promise<IdGenerator[]> {
    return lastValueFrom(
      this.httpClient
        .get<
          IdGenerator[]
        >(this.data[0].url + "/configuration/idGenerators", { headers: new HttpHeaders().set("mainzellisteApiVersion", "3.2") })
        .pipe(
          catchError((e) =>
            throwError(
              () =>
                new Error(
                  this.translate.instant(
                    "error.app_config_service_fetch_id_generators",
                  ),
                ),
            ),
          ),
  ...
  public fetchMainzellisteAssociatedIdGenerators(): Promise<IdGenerator[]> {
    return lastValueFrom(
      this.httpClient
        .get<AssociatedIds>(
          this.data[0].url + "/configuration/idGenerators/associatedIds",
        ...
        .pipe(
          ...
          map((associatedIds) => {
            ...
            for (let key in associatedIds) {
              this.mainzellisteAssociatedIdGenerators.push(
                ...associatedIds[key],
              );
              this.mainzellisteAssociatedIdGeneratorsMap.set(
                key,
                associatedIds[key],
              );
            }
  ...
  public fetchClaims(): Promise<ClaimsConfig[]> {
    return firstValueFrom(
      this.httpClient
        .get<ClaimsConfig[]>(this.data[0].url + "/configuration/claims", {
          headers: new HttpHeaders().set("mainzellisteApiVersion", "3.2"),
        ...
        .pipe(
          catchError((e) =>
            throwError(
              () =>
                new Error(
                  "Can't init claims configurations. Failed to connect " +
  ...
  readConsentTerminology() {
    return firstValueFrom(
      this.httpClient
        .get<ConsentTerminology>(
          "assets/consent/mii-broad-consent-versions.json",
        )
        .pipe(
          catchError((e) =>
            throwError(
  ...
  public fetchMainzellisteFields(): Promise<MainzellisteField[]> {
    ...
    return lastValueFrom(
      this.httpClient
        .get<
          MainzellisteField[]
        >(fieldEndpointUrl, { headers: new HttpHeaders().set("mainzellisteApiVersion", "3.2") })
        .pipe(
          catchError((e) =>
            throwError(
              () =>
                new Error(
                  this.translate.instant(
                    "error.app_config_service_fetch_fields",
                  ) + fieldEndpointUrl,
                ),
            ),
          ),
          map((mlFields) => {
            ...
            for (let configuredField of this.data[0].fields) {
              ...
              if (configuredField.mainzellisteFields != undefined) {
                ...
                // init other fields
                this.initField(
                  configuredField.mainzellisteField,
                  configuredField,
                  mlFields,
                );
              }
  ...
  private initField(
    ...
    backendMlField: MainzellisteField[],
    isDateType?: boolean,
  ) {
    // find backend field configuration
    let mlField: MainzellisteField | undefined = backendMlField.find(
      (f) => f.name == fieldName,
    );
    if (mlField == undefined)
      throw new Error(
        this.translate.instant(
          "error.app_config_service_field_not_defined_text1",
        ) +
          fieldName +
          this.translate.instant(
            "error.app_config_service_field_not_defined_text2",
          ),
      );
    ...
    if (!isDateType) {
      if (
        ["sex", "gender", "geschlecht"].includes(mlField.name.toLowerCase())
      ) {
        ...
        throw new Error(
          this.translate.instant(
            "error.app_config_service_type_not_supported_text1",
          ) +
            fieldName +
            this.translate.instant(
              "error.app_config_service_type_not_supported_text2",
            ) +
            mlField.type +
            this.translate.instant(
              "error.app_config_service_type_not_supported_text3",
            ),
        );
    }
    ...
    else configuredField.validator = mlField.validation ?? "";
  ...
  public validateMainIdType(idGenerators: IdGenerator[]) {
    ...
    if (
      config.mainIdType != undefined &&
      !idGenerators.some((g) => g.idType == config.mainIdType?.trim())
    ) {
      throw new Error(
        "mainIdType '" +
          config.mainIdType +
          "'not configured in the backend, please check your ui configuration",
      );
    }
  ...
  private static validateOAuthConfig(config: OAuthConfig | undefined): boolean {
    return (
      config !== undefined &&
      !AppConfigService.isStringEmpty(config.url) &&
