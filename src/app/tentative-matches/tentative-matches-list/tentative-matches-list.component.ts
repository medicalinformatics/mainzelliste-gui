...
import {
  SplitUnsureMatchDialogComponent
} from "./split-unsure-match-dialog-component/split-unsure-match-dialog.component";
...
export class TentativeMatchesListComponent implements OnInit {
  ...
  constructor(
    ...
    public resolveDialog: MatDialog,
  ) {
  ...
  ngOnInit(): void {
    ...
    this.addColumn({id: "tentative", name: "tentative", type: "empty", i18n: "read_tentatives.tentativeMatch", rowSpan: 1, colSpan: fields.length + 2}, this.header1)
    fields.forEach(f =>this.addColumn(
      {id: "p." + f.semantic,  name: "p." + f.name, type: "text", i18n: f.i18n, rowSpan: 1, colSpan: 1}, this.header2)
    )
    this.addColumn({id: "p.tenants", name: "p.tenants", type: "text", i18n: "patientlist.headers_tenants", rowSpan: 1, colSpan: 1}, this.header2)
    ...
    this.addColumn({id: "resolve", name: "id", type: "resolveButton", i18n: "", rowSpan: 2, colSpan: 1}, this.header1)

    this.addColumn({id: "bestMatch", name: "bestMatch", type: "empty", i18n: "read_tentatives.bestMatch", rowSpan: 1, colSpan: fields.length + 2}, this.header1)
    fields.forEach(f => this.addColumn(
      {id: "b." + f.semantic, name: "b." + f.name, type: "text", i18n: f.i18n, rowSpan: 1, colSpan: 1}, this.header2)
    )
    this.addColumn({id: "b.tenants", name: "b.tenants", type: "text", i18n: "patientlist.headers_tenants", rowSpan: 1, colSpan: 1}, this.header2)
  ...
  openResolveDialog(tentativeMatchId: number): void {
    this.resolveDialog.open(SplitUnsureMatchDialogComponent, {
      data: {
        callbackObservable: this.patientListService.resolveTentative(tentativeMatchId, SolveTentativeOperationType.split),
        returnError: false
      },
    })
